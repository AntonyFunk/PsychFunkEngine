...
class Language
{
	...
	inline public static function getFileTranslation(key:String)
	{
		...
		var newKey:String = key.trim().toLowerCase();
		var ext:String = newKey.extension();

		var phrase:String = phrases.get(newKey.withoutExtension());
		if (phrase != null)
		{
			if (ext != null && ext.length > 0) return 'langs/$phrase.$ext';
			else return 'langs/$phrase';
		}
		#end
