...
class FunkinSoundTray extends FlxSoundTray
{
	...
	override public function update(ms:Float):Void
	{
		...
		else if (!visible) showTray();

		y = FlxMath.lerp(y, lerpYPos, .1 * FlxG.elapsed * 60);
		alpha = FlxMath.lerp(alpha, alphaTarget, .25 * FlxG.elapsed * 60);
		screenCenter();
	}
	...
	function updateBars():Void
	{
		...
		for (i => bar in _bars) bar.visible = i < globalVolume;
	}
