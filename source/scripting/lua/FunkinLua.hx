...
class FunkinLua implements FunkinScript {
	...
	public function set(variable:String, data:Dynamic):Dynamic {
		if (lua == null)
			return null;
	...
	public static function implement():Void {
		...
		registerFunction('loadMultipleFrames', function(variable:String, images:Array<String>) {
			...
			if (object != null && images != null && images.length > 0)
				LuaUtils.loadMultipleFrames(object, images);
		});
		...
		registerFunction('makeAnimatedLuaSprite', function(tag:String, ?image:Dynamic = null, ?x:Float = 0, ?y:Float = 0, ?spriteType:String = 'auto') {
			tag = tag.replace('.', '');
			LuaUtils.destroyObject(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);

			if (image != null && image.length > 0) {
				if ((spriteType == 'multiatlas' || spriteType == 'multipleatlas') && Std.isOfType(image, Array)) 
					LuaUtils.loadMultipleFrames(leSprite, image);
				else 
