...
class FlxCamera extends FlxBasic
{
	...
	/**
	 * Internal, camera's `flash()` special effect ease.
	 */
	var _fxFlashEase:EaseFunction;

	/**
	 * Internal, used to control the `flash()` duration.
	 */
	var _fxFlashElapsed:Float = 0;
	...
	function updateFlash(elapsed:Float):Void
	{
		...
		{
			_fxFlashElapsed += elapsed;

			if (_fxFlashEase != null)
			{
				var t = _fxFlashElapsed / _fxFlashDuration;
				if (t > 1.0) t = 1.0;

				_fxFlashAlpha = 1.0 - _fxFlashEase(t);
			}	
			else _fxFlashAlpha -= elapsed / _fxFlashDuration;

			if (_fxFlashElapsed >= _fxFlashDuration)
			{
				if (_fxFadeComplete != null) _fxFlashComplete();
			}
		}
	}
	...
	function updateScrollRect():Void
	{
		...
		var offset = flashSprite.cacheAsBitmap ? 0 : 1;
		var rectWidth = width * initialZoom * FlxG.scaleMode.scale.x;
		var rectHeight = height * initialZoom * FlxG.scaleMode.scale.y;

		rect.x = rect.y = 0;

		rect.width = rectWidth - offset;
		rect.height = rectHeight - offset;

		_scrollRect.scrollRect = rect;

		_scrollRect.x = -0.5 * rectWidth;
		_scrollRect.y = -0.5 * rectHeight;
	}
	 ...
	 */
	public function flash(Color:FlxColor = FlxColor.WHITE, Duration:Float = 1, ?Ease:EaseFunction, ?OnComplete:Void->Void, Force:Bool = false):Void
	{
	...
	{
		flashSprite.filters = filtersEnabled ? Value : null;
		onResize();
